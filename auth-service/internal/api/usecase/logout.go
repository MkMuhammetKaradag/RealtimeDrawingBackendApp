...
func (u *logoutUseCase) Execute(fbrCtx *fiber.Ctx, ctx context.Context) (int, error) {
	...
	if err := u.sessionManager.DeleteSession(ctx, cookieSessionToken); err != nil {
		...
		default:
			return fiber.StatusInternalServerError, fmt.Errorf("failed to delete session: %w", err)
		}
